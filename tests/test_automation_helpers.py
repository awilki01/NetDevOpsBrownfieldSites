...
class TestAutomationHelpers:
    ...
    def test_if_fails_when_directory_not_present(self) -> None:
        """Ensure failure if directory structure is not in place"""
        with pytest.raises(FileNotFoundError):
            del_directory_contents(
                ["./bad_directory", "./bad_directory2", "./bad_directory3"]
            )

    def test_directory_structure(self) -> None:
        """Ensure proper directory structure is in place"""
        result = (
            os.path.isdir("./remediation_config_changes")
            and os.path.isdir("./rendered_configs")
    ...
    def test_ensure_configs_are_properly_rendered(self, nr):
        ...
        for key in nr.inventory.hosts.keys():
            ...
            assert (
                "-BEGIN RENDER-" in rendered_config
                and "-END RENDER-" in rendered_config
            )
    ...
    def test_proper_configs_were_pushed_to_device(self, nr):
        ...
        for key in nr.inventory.hosts.keys():
            ...
            device_uuid = result[key][0].result["get_interfaces"]["Loopback1000"][
                "description"
            ]
